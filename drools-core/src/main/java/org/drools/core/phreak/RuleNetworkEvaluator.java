        ...
        if ( NodeTypeEnums.isBetaNode( node )) {
            ...
            switch (node.getType()) {
                ...
                case NodeTypeEnums.AsyncSendNode: {
                    pSendNode.doNode((AsyncSendNode) node, (AsyncSendMemory) nodeMem, sink,
                            agenda.getWorkingMemory(), srcTuples, trgTuples, stagedLeftTuples);
                    break;
                }
                case NodeTypeEnums.AsyncReceiveNode: {
                    pReceiveNode.doNode( (AsyncReceiveNode) node, (AsyncReceiveMemory) nodeMem, pmem, smem, sink, agenda.getWorkingMemory(), srcTuples, trgTuples, stagedLeftTuples);
                    break;
                }
            }
